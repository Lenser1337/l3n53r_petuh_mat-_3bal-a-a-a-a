...
module.exports.run = async (bot, message, args) => {
  ...
  message.channel.send(`Понял, принял! <@${tomute.id}> был замучен в канализации на ${ms(ms(mutetime))}`);

  const embed = new Discord.RichEmbed()
  .setTitle(":star: Отчет о муте в канализации :star:")
  .setColor("#fc6400")
  ...
  setTimeout(function(){
    if(tomute.roles.has(muterole.id)){
      ...
      repchannel.send(`<@${tomute.id}> был размучен в канализации!`);
    }
  }, ms(mutetime));
